...
export const invoiceQualityDashboardSchema = z.object({
  ...
  totals: z.object({
    ...
    aiFilledEvents: z.number().int().nonnegative(),
    correctedEvents: z.number().int().nonnegative(),
    automaticEnrichmentEvents: z.number().int().nonnegative(),
    acceptedEvents: z.number().int().nonnegative(),
    correctionRate: z.number().min(0).max(1).nullable(),
    aiFilledRate: z.number().min(0).max(1).nullable(),
    automaticEnrichmentRate: z.number().min(0).max(1).nullable(),
  ...
  byMatchDecision: z.array(invoiceQualityBreakdownSchema),
  cases: z.array(
    z.object({
      draftId: z.string().uuid(),
      status: invoiceDraftStatusSchema,

...
async function upsertUser(
  ...
  if (existingUser) {
    ...
    // For new users, we need to create a default company or assign them to one
    // For now, create a default company for the user
    const company = await storage.createCompany({
      name: `${claims["first_name"] || "User"}'s Repair Shop`,
      plan: "basic"
    });
...
export const isAuthenticated: RequestHandler = async (req, res, next) => {
  // Support local email/password auth sessions
